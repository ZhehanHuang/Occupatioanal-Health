...
# long_short_split.iloc[:,1] = 0 #仅计算短期影响
long_short_split.iloc[:,2] = 0 #仅计算长期影响
...
# short_duration = pd.read_excel(r'.\data\support_data.xlsx', sheet_name=4, usecols='A:C') #GDB
...
np_short_duration = pd.Series(np_short_duration.iloc[:,0])  
'''Step5.2 导入长期伤害和疾病的持续时间'''
'''Step6.1 导入短期伤害和疾病的权重'''
short_weight = pd.Series(short_weight.iloc[:,1])    
'''Step6.2 导入长期伤害和疾病的权重'''
long_weight = pd.Series(long_weight.iloc[:,1])     
'''Step7.1 计算短期伤害和疾病的YLD'''
short_injuries = pd.DataFrame(np_industry_shortinj_age.sum(axis=2)).fillna(0) #沿年龄区间叠加
yld_short = pd.DataFrame(short_injuries * short_weight* np_short_duration)
...
for i in range(df_industry_nature.shape[0]): # 每个产业
    for j in range(df_industry_nature.shape[1]-2): # 每种非致命伤害
        yld_long.iloc[i,j+2] = sum(np_industry_longinj_age[i,j,:] * np_long_duration* long_weight[j])
...
cf_total_np = np.array(cf_total.iloc[:,1:])
...
cf_T = process_3_np.sum(axis=1)
eye = np.eye(404)
cf_n = eye*cf_T
cf_back = np.dot(i_A_inv.T, cf_n)

if process_4_np.sum(axis=1).all() == cf_back.sum(axis=1).all():
    print('Correct')
    
number_matrix = np.matrix(df_industry_nature.iloc[:,2:])
number_matrix[isnan(number_matrix)] = 0
number_CONV = np.dot(Conv_matrix.T,number_matrix)
