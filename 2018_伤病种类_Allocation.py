# -*- coding: utf-8 -*-
"""
Created on Mon Nov  8 01:00:24 2021
...
# 割伤,撕裂伤,刺穿伤（cuts, lacerations, punctures）类别下移除总计（Total）数据（方法为将未统计的平均分配给各项已统计数据）
...
# 移除总病例数（Total cases）；上一条中的总数（Total）；移除扭伤（With sprains）及骨折伤（With fractures），用多重外伤（Multiple traumatic injuries (Total)）即总计数代替
...
for p in range(coding_counting.shape[0]):
    ...
    coding_level6_fix_all.iloc[x:x+y,2:] = coding_level6.iloc[x:x+y,2:] + coding_average_all.iloc[p,2:]
